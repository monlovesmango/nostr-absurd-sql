...
async function populateFromRelay(filters, relays) {
  if (filters.length === 0 || relays.length === 0) {
    output('need 1 filter and 1 relay to populate from relay, exiting')
    return
  }
  output(`using filters ${JSON.stringify(filters)} with relays ${JSON.stringify(relays)}`)
  clearTimings();
  ...
  relayWorker = new Worker(new URL('./relay.worker.js', import.meta.url));
  let debouncedSaveEvents = mergebounce(
    events => q.saveEvents(db, events),
    500,
  ...
  relayWorker.postMessage({ type: 'sub', filters, relays });
  ...
  while (Date.now() - lastSave < 2000) {
    await new Promise((resolve) => setTimeout(resolve, 500));
    // if (!lastSave) lastSave = Date.now()
  }
...
function updateSubscriptionFilters(filters) {
  relayWorker.postMessage({ type: 'updateFilters', filters })
}

function updateSubscriptionRelays(relays) {
  relayWorker.postMessage({ type: 'updateRelays', relays })
}
    ...
    switch (msg.data.type) {
      case 'ui-invoke':
        ...
        switch (msg.data.name) {
          case 'populateFromRelay':
            methods[msg.data.name](msg.data.filters, msg.data.relays);
            break;

          case 'updateSubscriptionFilters':
            methods[msg.data.name](msg.data.filters);
            break;

          case 'updateSubscriptionRelays':
            methods[msg.data.name](msg.data.relays);
            break;

          default:
            methods[msg.data.name]();
        }
        // if (msg.data.name === 'populateFromRelay') {
        //   methods[msg.data.name](msg.data.filters, msg.data.relays);
        // } else if 
        // } else methods[msg.data.name]();
        break;
